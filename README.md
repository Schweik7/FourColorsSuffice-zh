...
一个 Vite + React 应用，把上面的校对流程和两个绘图工具合到了一起：

- **地图绘制台** —— 在六边形网格上直接涂出区域，**邻接关系是从涂色里量出来的，不用输入**。
  空洞、包围、飞地都是涂色的自然结果。「整形」在不改动邻接关系的前提下把形状磨圆、把面积拉匀：
  每挪一格都先验过邻接与连通性，所以怎么跑都不会把画好的拓扑弄坏。
  涂完描出平滑边界，交给下面同一套编辑与导出。
- **SVG 生成器** —— 给一张邻接图（可视化点选，或粘贴 NetworkX 风格的文本 / Python 代码），
  程序化生成对应的四色地图。现在主要用来给绘制台铺一张起始草稿。
...
之所以以"涂"为主而不是以"输入邻接关系"为主：给定邻接关系画出好看的地图，
在文献里叫 cartogram / rectilinear dual，而"每块都凸、都圆、都不带尖角"对一般平面图
根本无解——只能把"好看"当成能量去优化，而解析构造没有优化的余地。何况本项目的图是
照着扫描件重画的，构图早已定下，不需要算法去发明。
...
webapp/                    # 插图工作台：地图绘制台 + SVG 生成器 + 校对台
